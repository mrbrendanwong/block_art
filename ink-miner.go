 ...
 */
 ...
 // ShapeOp represents a shape operation
 type ShapeOp struct {
 }
 ...
 // SetHash sets the hash of the current block
 // TODO what is the right method to do this.. POW?
 func (b *Block) SetHash() {
     headers := bytes.Join([][]byte{b.BlockNumber, b.PrevBlockHash, b.Transactions,}, []byte{})
 ...
 // hash = strong of block's contents
 ...
     // check whether other miners exist
 
     // if not
     // first miner creates blockchain with genesis block
     BlockchainRef = NewBlockchain()
 
     // if other miners exist, ask them for their blockchains
     // pick the longest path
 
     // continuously listen for rpc calls from artnoder(blockartlib)
